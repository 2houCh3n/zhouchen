          ...
          <ul class="msgList">
            <li v-for="(item,index) in msgList" :key="'msg'+item.id">
              <div class="ask">
                ...
                <span class="tipsInfo">{{item.asker+' '+item.createTime}}</span>
              </div>
              <div class="answer">
                <span class="note">答</span>
                <span class="text">{{Object.keys(item.reply).length>0?item.reply.content:'暂时没有回答'}}</span>
                <span class="tipsInfo">{{Object.keys(item.reply).length>0?item.reply.replyTime:''}}</span>
              </div>
            </li>
          </ul>
        </div>
  ...
  methods:{
    ...
    getGoodsInfo(id){
      ...
      .then((data)=>{
        ...
        this.specs = data.specList;
        this.typeId = data.typeId;
        this.temSpecId = data.specList[0].id;
    ...
    postAsk(){
      ...
      .then(()=>{
        ...
        this.msgList.unshift({
          ...
          createTime:time.getMonth()+1+'-'+time.getDate(),
          reply:{}
        });
        this.askContent = '';
      })
