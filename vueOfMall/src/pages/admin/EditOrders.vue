<template>
  <div class="EditOrders">
    ...
    <div class="content">
      ...
      <div class="inputBox">
        <span>总金额：</span>
        <span class="val">{{'¥'+goodPrice}}</span>
      </div>
      <div class="inputBox">
        <span>规格：</span>
        <Radio v-for="(item,index) in specs" :key="item.id" v-model="temSpecId" :initVal="temSpecId" radioName="spec" :radioVal="item.id">
  ...
  methods:{
    fetchOrderDetail(id){
      ...
      .then((order)=>{
        ...
        this.temNum = Number(order.goodNum);
