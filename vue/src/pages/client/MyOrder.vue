      ...
      <ul class="orderList">
        <li v-for="(item,index) in orderList" :key="'order'+item.id">
          <div class="orderHeader">
            ...
            <span class="state">{{tagList[item.stateId+1]}}</span>
          ...
          <div class="orderDetail">
            ...
            <button v-if="item.stateId===0" @click="confirmPay(item.id)">确认付款</button>
            <button v-else-if="item.stateId===2" @click="confirmReceive(item.id)">确认收货</button>
            <button v-else-if="item.stateId===3 && !item.hasComment" @click="showPopup(item.id,item.goods.id,item.goods.specId)">评价</button>
            <span class="hasComment" v-else-if="item.stateId===3 && item.hasComment">已评价</span>
          </div>
        </li>
      </ul>
    </div>
  ...
  methods:{
    ...
    sendComment(){
      ...
      const res = sendComment({
        ...
        goodId:this.curCommentGoodsId,
        specId:this.curCommentGoodsDetailId,
